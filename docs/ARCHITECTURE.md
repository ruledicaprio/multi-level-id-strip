# 🏛️ Architectural Manifest: multi-level-id-strip (mlis) — Air-Gapped Document Processing (v0.7.5)
...
As of **v0.6.0**, the LLM fallback tier runs **in-process** — no Python interpreter, no gRPC sidecar, no second container required. As of **v0.7.0**, OCR runs in-process too — no Docker container required either, on any platform, for image input. As of **v0.7.5**, both legacy paths (the gRPC Tier-2 sidecar and the Docker-based `docling-serve` OCR engine) are removed entirely rather than merely non-default — which also means **PDF input is no longer supported**; `mlis` is image-only. Docker and Python are no longer required for *any* functional code path. This is the third concrete step on the road to a single static `musl` binary (see [§9](#9-strategic-roadmap-v080--v100-the-road-to-a-single-static-binary)); everything else in this document describes what's true *today*, in this repo, right now.
...
7. **`mlis doctor`:** preflight — for the `rust` OCR engine, checks the two `.rten` model files exist and SHA-256-verifies them; for `native`, no network check needed. Then calls `Pipeline::infer_health()`, which confirms the GGUF file exists and SHA-256-verifies it (or reports the skip).
...
7. Configuration via environment: `BIND_ADDR`, `MLIS_OCR_ENGINE`, `MLIS_MODEL_PATH`, `MLIS_MAX_QUEUE_DEPTH`, `MLIS_TOKEN`, `MLIS_AUDIT_LOG`, `MLIS_KEY`, `WORK_DIR`.
...
* **v0.9.0 — PII memory hardening + fuzzing.** `zeroize`/`ZeroizeOnDrop` on in-memory PII structures to shrink the window a swap file or crash dump could leak identity data; fuzz-testing the ingest path (image parsing, OCR Markdown parsing, MRZ repair) for the untrusted-input surface a sellable product needs hardened.
...
`docker/docker-compose.yml` still exists purely as an optional way to build/run the containerized `mlis-serve` image — it is no longer needed for any *functional* code path, unlike the legacy `docling`/`inferer` services it used to define.

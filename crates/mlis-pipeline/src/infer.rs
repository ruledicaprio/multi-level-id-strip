...
//! `mlis-llm` / `llama-cpp-2`. The legacy gRPC backend (talking to a Python
...
compile_error!("mlis-pipeline requires the `inferer-native` feature");
...
/// backend as of v0.7.5. `MLIS_INFERER` is still read for backward
...
pub fn backend_from_env() -> Box<dyn InferBackend> {
    if let Ok(choice) = std::env::var("MLIS_INFERER") {
        if choice != "native" {
            eprintln!(
                "[mlis] MLIS_INFERER={choice:?} is not a recognized backend (gRPC support \

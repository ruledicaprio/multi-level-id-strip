...
//! # Pass budget
//!
//! `MLIS_OCR_MAX_PASSES` (default [`DEFAULT_MAX_PASSES`]) and
//! `MLIS_OCR_MAX_SECONDS` (default [`DEFAULT_MAX_SECONDS`]) bound the retry
...
//! # Diagnostics
//!
//! Set `MLIS_OCR_VERBOSE=1` to log each pass's elapsed time and detected-
...
/// Default `MLIS_OCR_MAX_PASSES` ceiling on total OCR passes per document
...
/// worst case. The `MLIS_OCR_MAX_SECONDS` wall-clock budget is what actually
/// bounds a pathological document.
const DEFAULT_MAX_PASSES: usize = 7;

/// Default `MLIS_OCR_MAX_SECONDS` wall-clock ceiling on the whole
...
impl NativeOcr {
    ...
    pub fn recognize(&self, image_path: &Path) -> Result<String, String> {
        ...
        if verbose {
            ...
            eprintln!(
                "[mlis-ocr] general pass: {:?} elapsed, {regions} region(s) detected",
        ...
        if verbose {
            eprintln!("[mlis-ocr] Tier-1 miss on general pass; MRZ-band candidate lines:");
            for line in mrz_shaped_lines(&text).lines() {
                eprintln!("[mlis-ocr]   {line}");
            }
        }
        ...
        for (passes_run, (i, variant)) in (1usize..).zip(variants) {
            if passes_run >= max_passes {
                if verbose {
                    eprintln!(
                        "[mlis-ocr] pass budget ({max_passes}) reached before variant {i}; stopping retries"
                    );
                }
                break;
            }
            if overall_started.elapsed() >= max_duration {
                if verbose {
                    eprintln!(
                        "[mlis-ocr] time budget ({max_duration:?}) reached before variant {i}; stopping retries"
                    );
                }
                break;
            }
            ...
            let Ok(pass_text) = run_pass(&self.mrz_engine, &variant) else {
                if verbose {
                    eprintln!("[mlis-ocr] variant {i}: pass failed, skipping");
                }
                continue;
            };
            if verbose {
                ...
                eprintln!(
                    "[mlis-ocr] variant {i}: {:?} elapsed, {regions} region(s) detected",
            ...
            if candidates.is_empty() {
                if verbose {
                    eprintln!("[mlis-ocr] variant {i}: no MRZ-shaped lines");
                }
                continue;
            }
            text.push('\n');
            ...
            if has_valid_mrz(&candidates) {
                if verbose {
                    eprintln!("[mlis-ocr] variant {i}: valid MRZ found, stopping retries");
                }
                break;
            } else if verbose {
                eprintln!("[mlis-ocr] variant {i}: MRZ-shaped but checksum-invalid lines:");
...
/// Is `MLIS_OCR_VERBOSE=1` set? Gates the per-pass diagnostic logging
/// described in the module docs.
fn verbose_enabled() -> bool {
    std::env::var("MLIS_OCR_VERBOSE").as_deref() == Ok("1")
}

/// `MLIS_OCR_MAX_PASSES`, or [`DEFAULT_MAX_PASSES`] if unset/invalid/zero.
fn max_passes() -> usize {
    std::env::var("MLIS_OCR_MAX_PASSES")
        .ok()
...
/// `MLIS_OCR_MAX_SECONDS`, or [`DEFAULT_MAX_SECONDS`] if unset/invalid/zero.
fn max_duration() -> Duration {
    std::env::var("MLIS_OCR_MAX_SECONDS")
        .ok()
...
/// Detected text-region count for one pass, for `MLIS_OCR_VERBOSE` logging
...
mod tests {
    ...
    fn max_passes_reads_env_with_fallback() {
        unsafe { std::env::remove_var("MLIS_OCR_MAX_PASSES") };
        assert_eq!(max_passes(), DEFAULT_MAX_PASSES, "unset falls back");

        unsafe { std::env::set_var("MLIS_OCR_MAX_PASSES", "2") };
        assert_eq!(max_passes(), 2, "valid override honored");

        unsafe { std::env::set_var("MLIS_OCR_MAX_PASSES", "not-a-number") };
        assert_eq!(max_passes(), DEFAULT_MAX_PASSES, "invalid falls back");

        unsafe { std::env::set_var("MLIS_OCR_MAX_PASSES", "0") };
    ...
    fn max_duration_reads_env_with_fallback() {
        unsafe { std::env::remove_var("MLIS_OCR_MAX_SECONDS") };
        assert_eq!(
            max_duration(),
        ...
        unsafe { std::env::set_var("MLIS_OCR_MAX_SECONDS", "5") };
        assert_eq!(
            max_duration(),
        ...
        unsafe { std::env::set_var("MLIS_OCR_MAX_SECONDS", "0") };
        assert_eq!(
            max_duration(),
            Duration::from_secs(DEFAULT_MAX_SECONDS),
            "zero falls back"
    ...
    fn verbose_enabled_only_on_exact_flag() {
        unsafe { std::env::remove_var("MLIS_OCR_VERBOSE") };
        assert!(!verbose_enabled());
        unsafe { std::env::set_var("MLIS_OCR_VERBOSE", "true") };
        assert!(
            !verbose_enabled(),
            "only the literal \"1\" should enable it"
        );
        unsafe { std::env::set_var("MLIS_OCR_VERBOSE", "1") };
        assert!(verbose_enabled());
        unsafe { std::env::remove_var("MLIS_OCR_VERBOSE") };
    }
}

...
//! two already-downloaded `.rten` model files (via `MLIS_OCR_MODEL_DIR`,
...
fn main() {
    ...
    // directory (crates/mlis-ocr), not the workspace root — unlike the
    // runtime `MLIS_OCR_MODEL_DIR` default in download.rs, which resolves
    ...
    let workspace_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../..")
        .canonicalize()
        .expect("workspace root (two levels up from crates/mlis-ocr) exists");
    let model_dir: PathBuf = std::env::var("MLIS_OCR_MODEL_DIR")
        .map(PathBuf::from)
    ...
    embed_one(
        ...
        "MLIS_OCR_DETECTION_SHA256",
        KNOWN_GOOD_SHA256_DETECTION,
    );
    embed_one(
        &model_dir.join("text-recognition.rten"),
        &out_dir.join("text-recognition.rten"),
        "MLIS_OCR_RECOGNITION_SHA256",
        KNOWN_GOOD_SHA256_RECOGNITION,
    );

    println!("cargo:rerun-if-env-changed=MLIS_OCR_MODEL_DIR");
    println!("cargo:rerun-if-env-changed=MLIS_OCR_DETECTION_SHA256");
    println!("cargo:rerun-if-env-changed=MLIS_OCR_RECOGNITION_SHA256");
}
    ...
    let bytes = std::fs::read(src).unwrap_or_else(|e| {
        panic!(
            "embedded-models: could not read {} ({e}) — run the model download step \
             ...
             step) or set MLIS_OCR_MODEL_DIR to point at an existing copy",
            src.display()
        )
    });

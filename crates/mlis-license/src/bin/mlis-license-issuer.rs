...
//! `crates/mlis-license/src/sign.rs`'s module docs for why this is a
//! separate binary rather than a subcommand of the shipped `mlis` CLI).
//!
//! ```text
//! mlis-license-issuer keygen
//! mlis-license-issuer issue-license --customer "Acme Hospital" --tier enterprise \
//!     --expires-in-days 365 [--hw <fingerprint>] [--features extract,decrypt] \
//!     [--license-id lic-001] [--out license.mlis]
//! ```

use mlis_license::sign;
use mlis_license::LicensePayload;
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        ...
        _ => {
            eprintln!("Usage:");
            eprintln!("  mlis-license-issuer keygen");
            eprintln!("  mlis-license-issuer issue-license --customer <name> --tier <tier> --expires-in-days <n> [--hw <fingerprint>] [--features a,b,c] [--license-id <id>] [--out <path>]");
            std::process::exit(1);
        }
    }
}
...
fn keygen_command() {
    ...
    println!();
    println!(
        "Private key (KEEP OFFLINE, NEVER COMMIT — set as MLIS_LICENSE_PRIVKEY to issue licenses):"
    );
    println!("{priv_b64}");
    println!();
    println!("Public key (safe to publish — embed in crates/mlis-license/pubkey.b64):");
    println!("{pub_b64}");
}
...
fn issue_license_command(args: &[String]) {
    ...
    let Ok(privkey_b64) = env::var("MLIS_LICENSE_PRIVKEY") else {
        eprintln!(
            "❌ set MLIS_LICENSE_PRIVKEY (base64 private key from `keygen`) to issue licenses"
        );
        std::process::exit(1);
    };
    let signing_key = match sign::signing_key_from_base64(&privkey_b64) {
        Ok(k) => k,
        Err(e) => {
            eprintln!("❌ MLIS_LICENSE_PRIVKEY: {e}");
            std::process::exit(1);
        }
    };

...
//! feature — never compiled into the customer-facing `mlis`/`mlis-serve`
//! binaries, so they carry no private-key handling or keygen RNG dependency
//! at all. Only [`bin/mlis-license-issuer.rs`](../src/bin/mlis-license-issuer.rs)
...
mod tests {
    ...
    fn generated_keypair_round_trips_through_issue_and_verify() {
        // Verifies against the key directly, not via MLIS_LICENSE_PUBKEY —

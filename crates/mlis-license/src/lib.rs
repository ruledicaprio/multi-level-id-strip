...
//! shipped `mlis`/`mlis-serve` binary can be sold and metered for air-gapped
...
pub struct LicensePayload {
    ...
    /// A license can refuse to unlock an `mlis` build older than it was
...
/// against `MLIS_LICENSE_PUBKEY` if set, else the embedded key (see
...
/// exactly the way `mlis-serve`'s existing `startup_refusal` is tested.
pub fn check(

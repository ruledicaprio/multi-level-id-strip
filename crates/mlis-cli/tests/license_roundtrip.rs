//! Black-box tests for `mlis fingerprint` / `mlis verify-license` / the
...
//! keypair (NOT via `mlis-license`'s `vendor` feature — `mlis-cli` must
//! never build with that active, in any mode, so the shipped binary's
//! dependency tree stays provably signing-free) and spawns the built `mlis`
...
use mlis_license::{LicensePayload, SignedLicense, FORMAT_VERSION};
...
fn write_license_fixture(name: &str, signed: &SignedLicense) -> TempFileGuard {
    let path =
        std::env::temp_dir().join(format!("mlis-cli-test-{name}-{}.mlis", std::process::id()));
...
fn fingerprint_is_stable_and_non_empty() {
    let run = || {
        Command::new(env!("CARGO_BIN_EXE_mlis"))
            .arg("fingerprint")
            .output()
            .expect("run `mlis fingerprint`")
    };
    let (a, b) = (run(), run());
...
fn verify_license_accepts_a_valid_unbound_license() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_mlis"))
        ...
        .env("MLIS_LICENSE_PUBKEY", &pubkey_b64)
        .output()
        .expect("run `mlis verify-license`");
...
fn extraction_path_refuses_expired_license_but_skip_bypasses_the_gate() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_mlis"))
        ...
        .env("MLIS_LICENSE_PUBKEY", &pubkey_b64)
        .env("MLIS_LICENSE_PATH", license_fixture.0.to_str().unwrap())
        .output()
        .expect("run `mlis <file>`");
    assert!(
    ...
    // With MLIS_LICENSE_SKIP=1: the gate is bypassed. It will still fail
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_mlis"))
        ...
        .env("MLIS_OCR_AUTO_DOWNLOAD", "0")
        .env("MLIS_OCR_MODEL_DIR", &empty_model_dir)
        .output()
        .expect("run `mlis <file>` with MLIS_LICENSE_SKIP=1");

    std::fs::remove_dir_all(&empty_model_dir).ok();

    assert!(
        !String::from_utf8_lossy(&output.stderr).contains("license check failed"),
        "MLIS_LICENSE_SKIP=1 should bypass the license gate entirely, got: {}",
        String::from_utf8_lossy(&output.stderr)
    );
}
